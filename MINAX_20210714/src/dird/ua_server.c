...
static void *handle_UA_client_request(void *arg)
{
   ...
   /*Heart beat*/
   if (strncmp("*Hello DIRD*",user->msg,12) == 0)
   {
      char * name = (char *)malloc(strlen(user->msg) - 12);
      char * tmp = user->msg + 13;
      strcpy(name,tmp);
      P(fd_ip_mutex);
      if (dictAdd(fd_ip_dict, name, user) != 0)
      {
         Dmsg2(100, "Add to fd_ip_dict failed!from %s:%d\n",user->host(), user->port());
      }
      else
      {
         Dmsg3(100, "Add to fd_ip_dict Sucess!name:%s,%s:%d\n",name,user->host(), user->port());
         ua->UA_sock = NULL;
      }
      V(fd_ip_mutex);
      goto getout;
