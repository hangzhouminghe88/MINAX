...
extern void start_fd_dict_ctrl_thread();
extern void stop_fd_dict_ctrl_thread();
...
static bool make_pid_file = false;     /* create pid file */
static void init_reload(void);
static CONFIG *config;
static bool test_config = false;
dict *fd_ip_dict = NULL;
pthread_mutex_t fd_ip_mutex = PTHREAD_MUTEX_INITIALIZER;
...
int main (int argc, char *argv[])
{
   ...
   fd_ip_dict = dictCreate(&dtype, NULL);
...
void terminate_dird(int sig)
{
   ...
   lmgr_cleanup_main();
   stop_fd_dict_ctrl_thread();
   dictRelease(fd_ip_dict);
   sm_dump(false);
   exit(sig);
}
