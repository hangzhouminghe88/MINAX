                           ...
                           int verbose)
{
   ...
   if (!is_bsock_open(jcr->file_bsock)) {
      ...
      if (jcr->client->allow_fd_connections) {
         ...
         const char * clinet_name = jcr->client->hdr.name;
         P(fd_ip_mutex);
         dictEntry *entry = dictFind(fd_ip_dict, clinet_name);
         if (entry == NULL) {
            Dmsg1(DT_NETWORK, "FD client :%s Not Found!\n",clinet_name);
            V(fd_ip_mutex);
            return 0;
         }
         else{
            fd = jcr->file_bsock = dictGetVal(entry);
            dictDelete(fd_ip_dict, clinet_name);
            V(fd_ip_mutex);
         }
